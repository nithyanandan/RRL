...
with open(os.path.dirname(os.path.abspath(__file__))+'/rrl/githash.txt', 'w+') as githash_file:
    githash_file.write(githash)

metafile = open(os.path.dirname(os.path.abspath(__file__))+'/rrl/__init__.py').read()
...
setup(name='RRL',
    ...
    package_data={'rrl': ['*.txt', 'examples/simparms/*.yaml',
                          'examples/schedulers/*.txt',
                          'examples/dbparms/*.yaml',
                          'examples/ioparms/*.yaml', 'data/*.txt', 'data/*.dat',
